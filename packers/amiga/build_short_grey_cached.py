...
<div id="loading-screen">
    ...
    <div class="ls-title">The Short Grey</div>
...
html += f'<div id="b64-disk3" style="display:none">{disk3_b64}</div>\n'
html += f'<div id="b64-disk4" style="display:none">{disk4_b64}</div>\n'
html += '</div>\n'
...
html += """\n(async function() {
    ...
    if (cached) {
        ...
        window._DISK3_DATA = new Uint8Array(cached.disk3);
        window._DISK4_DATA = new Uint8Array(cached.disk4);
        ...
        setProgress(54, 'Decoding Disk 3...');
        var d3El = document.getElementById('b64-disk3');
        window._DISK3_DATA = await fastDecode(d3El.textContent);
        d3El.textContent = ''; d3El.remove();
        console.log('Disk 3 decoded:', window._DISK3_DATA.length, 'bytes');

        setProgress(66, 'Decoding Disk 4...');
        var d4El = document.getElementById('b64-disk4');
        window._DISK4_DATA = await fastDecode(d4El.textContent);
        d4El.textContent = ''; d4El.remove();
        ...
        if (db) {
            try {
                await dbPut(db, BUILD_HASH, {
                    ...
                    disk3: window._DISK3_DATA.buffer.slice(0),
                    disk4: window._DISK4_DATA.buffer.slice(0),
                });
